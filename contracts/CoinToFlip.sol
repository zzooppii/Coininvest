...
contract CoinToFlip {
    ...
    event Payment(address indexed beneficiary, uint amount);//상금전송 이벤트
    event FailedPayment(address indexed beneficiary, uint amount);//상금전송 실패이벤트
    ...
    // Funds withdrawal to maintain the house //돈 인출하는것
    ...
    //fallback 함수 하우스 이더를 받는 함후 => 초기자금 만들려고 만든거
    ...
    function placeBet(uint8 betMask) external payable {
        ...
        Bet storage bet = bets[msg.sender]; //mapping bets(address => Bet) 상태변수를 가르키는 포인터(플레이어를 장부에 기록해서 찾아라 있으면 쓰고 없으면 새로만듬)
    ...
    function revealResult() external {
        ...
        //베팅 blocknumber가 결과 blocknumber보다 작아야함 -> 베팅이 먼저 그 후 결과
    ...
    //인출할때 or 상금을 줄때씀 (send라는 함수는 true or false를 리턴함)
